...
export const PdfViewerModal: React.FC<PdfViewerModalProps> = ({ doc, orgName, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden text-slate-900">
        {/* Modal Header */}
        <div className="p-4 sm:p-5 bg-gradient-to-r from-amber-50/70 via-slate-50 to-white border-b border-slate-200 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-amber-100 border border-amber-300 flex items-center justify-center text-amber-800 shadow-2xs shrink-0">
              <FileText className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-black text-sm sm:text-base text-slate-900 font-title">{doc.titleFa}</h3>
                <span
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-amber-50 text-slate-800 hover:text-amber-900 text-xs font-bold rounded-xl border border-slate-300 hover:border-amber-300 transition-colors cursor-pointer"
              title="دانلود فایل"
            >
            ...
            <button
              onClick={onClose}
              className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-900 rounded-xl transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* Viewer Body */}
        <div className="flex-1 overflow-y-auto p-6 bg-slate-100/70 flex flex-col items-center justify-center min-h-[380px]">
          {doc.fileDataUrl ? (
            <div className="w-full h-[60vh] bg-white rounded-2xl overflow-hidden border border-slate-300 shadow-inner">
              <iframe
                src={doc.fileDataUrl}
                title={doc.titleFa}
            ...
            <div className="w-full max-w-2xl bg-white text-slate-900 p-8 rounded-2xl shadow-lg border border-slate-200 relative font-sans space-y-6">
              ...
              <div className="pt-4 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-500">
                <span>سامانه یکپارچه و پورتال رسمی شرکت‌های حمل‌ونقل</span>
