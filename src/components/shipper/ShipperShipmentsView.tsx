...
export const ShipperShipmentsView: React.FC<ShipperShipmentsViewProps> = ({
  ...
  const openDocumentViewer = (
    ...
    const pkg = generateFullShipmentDocumentPackage(load, currentOrg);
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {activeTab === 'edocs' && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white p-5 rounded-2xl border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-amber-500/20 border border-amber-400/30 flex items-center justify-center text-amber-400 font-bold shrink-0">
                <FileCheck className="w-6 h-6" />
              </div>
              <div>
                <h3 className="font-bold text-sm sm:text-base font-title flex items-center gap-2">
                  <span>سامانه تولید و صدور اسناد الکترونیکی محموله‌ها</span>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400">نمایش بر اساس بارنامه:</span>
              <select
                aria-label="انتخاب محموله جهت مشاهده اسناد"
                value={selectedLoad?.id || ''}
                onChange={(e) => {
                  const found = currentLoads.find((l) => l.id === e.target.value);
                  if (found) setSelectedLoad(found);
                }}
                className="bg-slate-800 border border-slate-700 text-white text-xs rounded-xl px-3 py-2 outline-hidden focus:border-amber-500 font-mono"
              >
                {currentLoads.map((load) => (
                  <option key={load.id} value={load.id}>
                    {load.billOfLadingNo} ({load.originCity} ➔ {load.destCity})
                  </option>
                ))}
              </select>
            </div>
          ...
          {(() => {
            const currentPackage = generateFullShipmentDocumentPackage(selectedLoad, currentOrg);
            return (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
