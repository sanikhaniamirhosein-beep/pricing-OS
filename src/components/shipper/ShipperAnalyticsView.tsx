...
export const ShipperAnalyticsView: React.FC = () => {
  ...
  const [activeViewMode, setActiveViewMode] = useState<'overview' | 'ai_advisor' | 'savings_matrix'>('overview');
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="font-bold text-slate-800 text-lg font-display">
                  گزارش‌ها و هوشمندی هزینه لجستیک ({currentShipperOrg.nameFa})
                </h1>
                <span className="bg-teal-50 text-teal-800 border border-teal-200 px-2.5 py-0.5 rounded-full text-xs font-mono font-bold flex items-center gap-1">
                  <Sparkles className="w-3 h-3 text-teal-600" />
                  مجهز به مشاور هوش مصنوعی اختصاصی
                </span>
              </div>
              <p className="text-slate-500 mt-1 text-xs">
                تحلیل هزینه‌ها، تطبیق مدارک حقوقی و دریافت توصیه‌های کاهش کرایه با ممیزی کامل اسناد شرکتی
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl border border-slate-200 self-start md:self-auto">
          ...
          >
            ...
            <span>فرصت‌های صرفه‌جویی ({savingsCards.length})</span>
          </button>
        </div>
      </div>
      ...
      {activeViewMode === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-l from-teal-50/80 via-emerald-50/30 to-white border border-teal-200 p-4 rounded-2xl shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            >
              <span>مشاهده تحلیل هوش مصنوعی</span>
